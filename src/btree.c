...
void
zfs_btree_create(zfs_btree_t *tree,
    int (*compar)(const void *, const void *, void *), void *compar_arg,
    size_t size)
{
	zfs_btree_create_custom(tree,
	    compar, compar_arg, size, BTREE_LEAF_SIZE);
}
    ...
    const void *value, zfs_btree_index_t *where)
{
	...
	while (max > min) {
		...
		int comp = tree->bt_compar(cur, value, tree->bt_compar_arg);
		if (comp < 0) {
			min = idx + 1;
		} else if (comp > 0) {
			max = idx;
...
zfs_btree_find(zfs_btree_t *tree, const void *value, zfs_btree_index_t *where)
{
	...
	if (tree->bt_bulk != NULL) {
		...
		if (tree->bt_compar(last_leaf->btl_elems +
		    last_leaf->btl_hdr.bth_first * size, value,
		    tree->bt_compar_arg) <= 0) {
			/*
	 ...
	 */
	for (node = (zfs_btree_core_t *)tree->bt_root; (int32_t)depth < tree->bt_height;
...
zfs_btree_verify_order_helper(zfs_btree_t *tree, zfs_btree_hdr_t *hdr)
{
	...
	if (!zfs_btree_is_core(hdr)) {
		...
		for (uint32_t i = 1; i < hdr->bth_count; i++) {
			VERIFY3S(tree->bt_compar(leaf->btl_elems +
			    ...
			    leaf->btl_elems +
			    (hdr->bth_first + i) * size, tree->bt_compar_arg),
			    ==, -1);
		}
		return;
	}
	...
	for (uint32_t i = 1; i < hdr->bth_count; i++) {
		VERIFY3S(tree->bt_compar(node->btc_elems + (i - 1) * size,
		    node->btc_elems + i * size, tree->bt_compar_arg), ==, -1);
	}
	for (uint32_t i = 0; i < hdr->bth_count; i++) {
		...
		int comp = tree->bt_compar(node->btc_elems + i * size,
		    left_child_last, tree->bt_compar_arg);
		if (comp <= 0) {
		...
		comp = tree->bt_compar(node->btc_elems + i * size,
		    right_child_first, tree->bt_compar_arg);
		if (comp >= 0) {
			panic("btree: compar returned %d (expected -1) at "
			    "%px %d: compar(%px,  %px)", comp, node, i,
